...
import { CIN, NationalID } from '../src/nationalid/ma/cin.js';
import { Gender } from '../src/nationalid/constant.js';

test('valid Moroccan CIN numbers', () => {
...
test('invalid Moroccan CIN numbers', () => {
  // Wrong length
...
test('invalid parse Moroccan CIN', () => {
...
test('with regexp', () => {
  ...
  assert.match('111111111111', CIN.METADATA.regexp);
  assert.match('000000000000', CIN.METADATA.regexp);
});

test('with metadata', () => {
  assert.ok(CIN.METADATA);
  assert.strictEqual(CIN.METADATA.iso3166_alpha2, 'MA');
  ...
  assert.strictEqual(CIN.METADATA.names.includes('Moroccan National ID'), true);
});
